...
export const Dialog: React.FC<DialogProps> = ({
  ...
  const typeStyles = {
    info: 'bg-primary-main hover:bg-primary-bright',
    ...
    >
      <div className="text-center">
        ...
        <div className="flex gap-3 justify-center">
          <button
            onClick={onClose}
            className="px-6 py-2.5 rounded-lg bg-glass-light text-text-primary
...
export const Drawer: React.FC<DrawerProps> = ({
  ...
  const sizeClasses = {
    ...
    large: position === 'left' || position === 'right' ? 'w-[500px]' : 'h-[500px]'
  }
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-background-primary/60 backdrop-blur-sm z-40"
          />
          
          {/* Drawer */}
          <motion.div
            initial={slideAnimation[position]}
            animate={{ x: 0, y: 0 }}
            exit={slideAnimation[position]}
            transition={{ type: 'spring', damping: 25 }}
            className={`
              fixed z-50 bg-background-secondary border-glass-border
